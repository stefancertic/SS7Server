...
static void *opsluzi_korisnika (void *arg)
{
  ...
  if ((errno = pthread_detach (pthread_self ())) != 0)
   ...
   printf("Server krece da gradi stek\n");
   izgradi_stek2(client);
   printf("Igradili smo stek!\n");  
	  ...
	  if (FD_ISSET (client, &read_set))
	    {
	      ...
	      printf("stigao je paket duzine %d\n", count);
...
main(int arg, char** argv) {
  ...
  int count;
   ...
   if (mysql_query(conn, "select * from providers where id=3")) {
      fprintf(stderr, "%s\n", mysql_error(conn));
      exit(1);
   }
   res = mysql_use_result(conn);
  ...
  address_server.sin_port = htons (2906);
  ...
  if (bind (server, (struct sockaddr *) &address_server, sizeof (struct sockaddr_in))< 0)
    error_fatal ("%s client bind() error: %s\n", argv[0], strerror (errno));
